...
RC Table::update_record(Trx *trx, Record *old_record, Record *new_record)
{
  ...
  if (rc != RC::SUCCESS) {
    record_handler_->update_record(old_record);
    insert_entry_of_indexes(old_record->data(), old_record->rid());
    return rc;
  }
  // TODO: update 事务支持
...
RC Table::update_entry_of_indexes(
    const char *old_record, const char *new_record, const RID &rid, const char *update_field)
{
  RC rc = RC::SUCCESS;
  for (Index *index : indexes_) {
    if (0 == strcmp(index->index_meta().field(), update_field)) {
